...
def parse_args():
  ...
  parser.add_argument('--target', type=str, default='mask_ours_3d, mask_lfw_3d,cfp_ff,cfp_fp,agedb_30,ours', help='')
  parser.add_argument('--memonger', action='store_true', default=True)
...
def train_net(args):
    ...
    def ver_test(nbatch):
      ...
      for i in range(len(ver_list)):
        acc1, std1, acc2, std2, xnorm, embeddings_list = verification.test(ver_list[i], model, args.batch_size, 10, None, None) # data_extra)
    ...
    def _batch_callback(param):
      ...
      if mbatch>=0 and mbatch%args.verbose==0:
        ...
        if len(acc_list)>0:
          ...
          #  if lfw_score>=0.8: #0.998
          #    do_save = True
          if acc_list[-1]>=highest_acc[-1]:
            highest_acc[-1] = acc_list[-1]
            if acc_list[-1]>=0.9: #0.99
              do_save = True
        if args.ckpt==0:
          do_save = False
        elif args.ckpt>1:
