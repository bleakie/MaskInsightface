...
class FaceImageIter(io.DataIter):
    ...
    # add blur
    def blur_aug(self, img, x):
        img = img.asnumpy()
        augs = ['GaussianBlur', 'MedianBlur', 'AverageBlur', 'MotionBlur']
        random.shuffle(augs)
        index = augs[0]
    ...
    def color_aug(self, img, x):
        # add aug with random of 2
        augs = [self.blur_aug, self.brightness_aug, self.contrast_aug, self.saturation_aug]
        random.shuffle(augs)
        for aug in augs[:2]:
            img = aug(img, x)
        return img
